...
export default function ArbitreMatches() {
  ...
  useEffect(() => {
    const fetchMatch = async () => {
      try {
        setIsLoading(true);
        // API call to get the arbitre's assigned match would go here
      ...
      <div className="w-full max-w-[390px] mx-auto space-y-4">
        ...
        <Card>
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-3">
              <button
                onClick={() => setCurrentMinute(Math.max(1, currentMinute - 1))}
                className="w-8 h-8 rounded-lg bg-[#F1EFE8] flex items-center justify-center"
              >
                <Minus size={16} />
              </button>
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                <div className="grid grid-cols-2 gap-3">
                  <Button variant="primary" onClick={() => addGoal('A')} className="w-full">
                    +1 Gol {match.teamA.split(' ')[0]}
                  </Button>
                  <Button variant="primary" onClick={() => addGoal('B')} className="w-full">
                    +1 Gol {match.teamB.split(' ')[0]}
                  </Button>
                </div>
              </div>
              ...
              <div>
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div className="space-y-2">
                    <Button variant="secondary" onClick={() => addCard('A', 'yellow')} className="w-full bg-[#FAEEDA] border-[#854F0B] text-[#854F0B]">
                      Groga {match.teamA.split(' ')[0]}
                    </Button>
                    <Button variant="secondary" onClick={() => addCard('A', 'red')} className="w-full bg-[#FCEBEB] border-[#A32D2D] text-[#A32D2D]">
                      Vermella {match.teamA.split(' ')[0]}
                    </Button>
                  </div>
                  <div className="space-y-2">
                    <Button variant="secondary" onClick={() => addCard('B', 'yellow')} className="w-full bg-[#FAEEDA] border-[#854F0B] text-[#854F0B]">
                      Groga {match.teamB.split(' ')[0]}
                    </Button>
                    <Button variant="secondary" onClick={() => addCard('B', 'red')} className="w-full bg-[#FCEBEB] border-[#A32D2D] text-[#A32D2D]">
                      Vermella {match.teamB.split(' ')[0]}
                    </Button>
                  </div>
                </div>
              </div>
        ...
        <Card>
          ...
          <div className="space-y-2 max-h-48 overflow-y-auto">
            {incidents.length === 0 ? (
              <p className="text-[13px] text-[#5F5E5A] text-center py-4">
                Encara no hi ha incidents registrats
              </p>
            ) : (
                ...
                <div key={i} className="p-3 bg-[#F1EFE8] rounded-lg flex items-center justify-between">
                  <div>
                    <p className="text-[13px] font-medium text-[#2C2C2A]">
                      Min {incident.minute}: {incident.type}
                    </p>
                    <p className="text-[12px] text-[#5F5E5A]">
                      {incident.team} {incident.player && `· ${incident.player}`}
                    </p>
                  </div>
                </div>
              ))
        ...
        <div className="fixed inset-0 bg-black/50 flex items-center justify-center p-4 z-50">
          <Card className="max-w-[350px] w-full">
            ...
            <p className="text-[#5F5E5A] mb-6">
              Estàs segur que vols tancar l'acta? Aquesta acció no es pot desfer.
            </p>
            <div className="space-y-2">
              <Button variant="primary" className="w-full" onClick={handleCloseMatch}>
                Sí, tancar acta
              </Button>
              <Button variant="ghost" className="w-full" onClick={() => setShowCloseConfirm(false)}>
                Cancel·lar
              </Button>
            </div>
          </Card>
